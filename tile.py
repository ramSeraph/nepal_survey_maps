...
if typ == 'main':
    ...
    INDEX_FILE = os.environ.get('INDEX_FILE', 'data/index_jica.geojsonl')
if INDEX_FILE != '':
    print('reading index file')
    with open(INDEX_FILE, 'r') as fp:
        for line in fp:
            f = json.loads(line)
            sheet_no = f['properties']['Name']
...
if __name__ == '__main__':
    tiles_dir = Path(f'export/tiles/{typ}')
    tiles_dir.mkdir(parents=True, exist_ok=True)

    file_list_file = Path(f'export/{typ}_files_to_tile.txt')

    file_names = list(glob.glob(f'export/gtiffs/{typ}/*.tif'))
    file_names = [ str(Path(f).resolve()) for f in file_names ]
    print(f' total files: {len(file_names)}')

    file_list_file.write_text('\n'.join(file_names))

    vrt_file = Path(f'export/{typ}_files_to_tile.vrt')

    prev_list_file = Path(f'export/{typ}_prev_files_to_tile.json')
