...
ONLY_DISK = os.environ.get('ONLY_DISK', '1') == '1'
...
ATTRIBUTION = '1:50000/1:25000 Topographic Maps from <a href="https://nationalgeoportal.gov.np/" target="_blank">Nepal Survey Department</a>, sourced through <a href="https://www.maze.com.np/Maps/Topo-Nepal" target="_blank">THE MAZE</a>'
...
def create_pmtiles(partition_info, reader):
    ...
    suffix_arr = []
    tiles_to_suffix = {}
    to_pmtiles_prefix = f'staging/pmtiles/Nepal_{typ}-'
    i = 0
    for suffix, data in partition_info.items():
    ...
    for suffix in suffix_arr:
        ...
        metadata = {
            ...
            "version": "1",
            "name": f"{typ}",
            "description": "Nepal Survey Department 1:50000/1:25000 Topographic Maps",
...
if __name__ == '__main__':
    ...
    to_partition_file = Path(f'staging/pmtiles/Nepal_{typ}.partition_info.json')
    if ONLY_DISK:
        reader = DiskSource(f'export/tiles/{typ}')
    else:
        reader = DiskAndPartitionedPMTilesSource('staging/tiles', f'export/pmtiles/Nepal_{typ}-', f'export/pmtiles/Nepal_{typ}.partition_info.json')
    ...
    pprint(mosaic_data)
    Path(f'staging/pmtiles/Nepal_{typ}.mosaic.json').write_text(json.dumps(mosaic_data))
